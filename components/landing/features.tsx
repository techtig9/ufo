...
export function Features() {
  return <section id="features" className="relative mx-auto max-w-7xl px-5 py-24 lg:px-8">
    <Reveal><div className="max-w-2xl"><span className="text-xs font-semibold uppercase tracking-[.18em] text-accent-alt-text">Everything in one workspace</span><h2 className="mt-4 font-display text-3xl font-semibold tracking-tight md:text-5xl">From first prompt to polished product.</h2><p className="mt-5 text-base leading-7 text-fg-muted">UFO combines generation, visual editing and product handoff into a single workflow built for shipping.</p></div></Reveal>
    <div className="mt-12 grid gap-4 md:grid-cols-2 lg:grid-cols-3">{FEATURES.map(([icon,title,body],i)=><Reveal key={title} delay={i*60}><Panel className="group h-full border-edge bg-surface-subtle p-6 transition duration-300 hover:-translate-y-1 hover:border-violet-400/20 hover:bg-surface-subtle"><span className="grid h-10 w-10 place-items-center rounded-xl bg-violet-500/10 text-lg text-accent-alt-text transition group-hover:bg-studio-citron/10 group-hover:text-brand-text">{icon}</span><h3 className="mt-6 text-base font-semibold">{title}</h3><p className="mt-2 text-sm leading-6 text-fg-faint">{body}</p><span className="mt-6 block h-px w-full bg-gradient-to-r from-white/10 to-transparent" /></Panel></Reveal>)}</div>
  </section>;
}
