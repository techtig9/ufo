...
async function callComplex(systemPrompt: string, userPrompt: string): Promise<string> {
  ...
  // Claude is the preferred provider for complex generation, but it is optional
  // for local development. If no Anthropic key is configured, use the existing
  // OpenAI-compatible cascade (Groq -> Cerebras -> OpenRouter) instead.
  if (!apiKey) {
    console.warn('[ai] ANTHROPIC_API_KEY is not set — using the available fallback AI provider.');
    return callSimple(
      systemPrompt,
