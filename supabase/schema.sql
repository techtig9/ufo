...
create table subscriptions (
  ...
  credits_remaining int not null default 1500,
...
create policy "insert own screen versions" on screen_versions for insert with check (
  exists (
    select 1 from screens s join projects p on p.id = s.project_id
    where s.id = screen_versions.screen_id and p.user_id = auth.uid()
  )
...
create policy "delete own screen versions" on screen_versions for delete using (
  exists (
    select 1 from screens s join projects p on p.id = s.project_id
    where s.id = screen_versions.screen_id and p.user_id = auth.uid()
  )
