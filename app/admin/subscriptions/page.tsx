      ...
      <table className="w-full text-left text-sm">
        ...
        <tbody className="divide-y divide-edge">
          {subscriptions?.map((s: any) => (
            <tr key={s.id}>
              <td className="px-4 py-3 text-fg-muted">{s.users?.email}</td>
              ...
              <td className="px-4 py-3">
                <form action={overrideSubscription} className="flex items-center gap-2">
                  ...
                  <select name="plan" defaultValue={s.plan} className="rounded border border-edge bg-surface-subtle px-2 py-1 text-xs">
                    <option value="free">Free</option>
                    <option value="starter">Starter</option>
                    <option value="pro">Pro</option>
                    <option value="business">Business</option>
                  </select>
                  <select name="status" defaultValue={s.status} className="rounded border border-edge bg-surface-subtle px-2 py-1 text-xs">
